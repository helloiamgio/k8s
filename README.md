...
![Alt text](https://github.com/helloiamgio/k8s/blob/main/k8s.gif)
...
kubectl get pods --all-namespaces -o custom-columns='POD_NAME:.metadata.name,NAMESPACE:.metadata.namespace,CLAIM_NAME:.spec.volumes[*].persistentVolumeClaim.claimName' | grep -v '<none>'
```
...
### Dump all applicative namespaces ###
```
kube-dump ns -d "$HOME/BACKUP-DR" -f -n "$(kubectl get ns -o jsonpath='{.items[*].metadata.name}' | tr ' ' '\n' | grep -Ev '^(openshift|kube)' | paste -sd, -)" -r deployments.apps,statefulsets.apps
```
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
## **CAPACITY**
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
